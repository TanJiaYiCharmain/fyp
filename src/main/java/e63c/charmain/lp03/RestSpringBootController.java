...
public class RestSpringBootController {
	...
	@GetMapping("/userData")
	public ModelAndView getChart(Model model) {
		// Call the /api/user endpoint to fetch user data
		List<Object> userData = getUser();

		// Process the user data to generate line chart data.
		Map<String, Integer> lineChartData = new LinkedHashMap<>(); // Use LinkedHashMap to preserve the order
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		...
		// Return ModelAndView with view name and model
		ModelAndView modelAndView = new ModelAndView("user_statistics");
		modelAndView.addObject("lineChartData", lineChartData);
		return modelAndView;
	}
